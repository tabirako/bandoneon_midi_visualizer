...
### `anglo-30-cg` (standard 30-button Anglo, C/G)

Transcribed 2026-09 from **two charts that agree note-for-note**, the same
two-source standard the bandoneon data was held to:

- the International Concertina Association's *Wheatstone/Lachenal C/G
  layout* (`concertina.org/concertina-systems/`) — the decisive one,
  because it carries Helmholtz octave marks **and** states `c' = Middle C`,
  which is what pins the octaves;
- `concertina.info`'s `c_g_30_wheatstone.jpg` — note names, no octaves.

Both are images, so this was read by eye (cropped and upscaled). What that
means for trust:

- **Note names: high confidence.** Two independent charts, all 30 buttons.
- **Octaves: from the ICA chart alone**, since the other gives none. They
  independently reproduce the arpeggio each diatonic row's push notes
  spell out — C row `C3 G3 C4 E4 G4 C5 E5 G5 C6 E6`, G row
  `B3 D4 G4 B4 D5 G5 B5 D6 G6 B6` — which is a real cross-check, and
  `instruments.test.js` asserts both sequences exactly. Range C3-B6.
- **One button was misread first time round.** The left G row's lowest
  button looked like `B/D` in the low-resolution `concertina.info` JPEG;
  zoomed, the ICA chart shows `b/a` (B3 push, A3 pull), matching a third
  chart (Terry Knight's, `korbo.com/piedcrow`). **B3/A3 is correct.** Worth
  knowing that reading these charts at 1:1 is not reliable.
- **Wheatstone/Lachenal, not Jeffries.** They differ in the accidental
  row — see "Open items".

**`x`/`y` are computed, not measured** (three rows of five on a gentle arc,
generated by the script in the file header) — an Anglo's rows are regular,
unlike the bandoneon's irregular diagonal packing, so no pixel-extraction
pipeline was needed. This is the saving predicted when the concertina work
was scoped.

**Both hands play at once** (`handSwitch: 'none'`): 15 buttons a hand fit
side by side on one keyboard — `Q W E R T | Y U I O P` (accidental),
`A S D F G | H J K L ;` (C row), `Z X C V B | N M , . /` (G row) — mirroring
how the instrument is actually held. Caps Lock has nothing to switch, so
its hint, the active-hand marker and the key-cap dimming are all hidden.
`assignKeyboardKeys()` also points `bassCodeMap` at `trebleCodeMap` for
these systems, so a stray Caps Lock press can't land on an empty map and
silently kill the keyboard.

This is also the first system to use an anchor set's `rowOffset`: with only
3 rows, the bottom-aligning `maxRow - usableRows + 1` clamps to 1 and would
...
21. **Phase 4: first non-bandoneon instrument — `anglo-30-cg`, 2026-09.**
    Data provenance and the two-chart verification are written up under
    "Data provenance"; the notable engine additions were `rowOffset` (a
    3-row instrument would otherwise be pushed onto the number row) and
    `handSwitch: 'none'`. That last one is the interesting one: the whole
    Caps Lock hand-switch exists because a bandoneon's 71 buttons can't fit
    on a keyboard, but an Anglo's 30 *can* — 15 a hand, one half of each
    row — so both hands play simultaneously, which is both closer to the
    real instrument and strictly better than switching. The metadata layer
    is what made that a per-system choice rather than an `if` in the
    keydown handler. The two bandoneons are untouched: their key
...
- **More concertina systems** (Phases 1-4 done — see Decision Log #18-21;
  ...
  - **A caution learned in Phase 4:** these charts are images and reading
    ...
    zoomed in. Always verify against two sources, and prefer one that
    states its octave convention.
